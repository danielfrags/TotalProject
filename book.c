...
void purchaseBook(Customer* customer, const char* bookTitle, const char* purchaseDate) {
    if (!customer) {
        printf("Invalid customer.\n");
        return;
    }

    Book* temp = head; // ראש הרשימה המקושרת של הספרים
    while (temp) {
        if (strcmp(temp->title, bookTitle) == 0) {
            if (temp->stock > 0) {
                // ניסיון להוספת הרכישה
                if (addPurchase(customer, bookTitle, purchaseDate)) {
                    temp->stock--;  // הפחתת מלאי הספר רק אם הרכישה הצליחה
                    printf("Purchase successful! '%s' has been added to %s's purchases on %s.\n", bookTitle, customer->name, purchaseDate);
                }
                return;
            }
            else {
