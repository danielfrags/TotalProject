...
void purchaseBook(Customer* customer, const char* bookTitle) {
    if (!customer) {
    ...
    while (temp) {
        if (strcmp(temp->title, bookTitle) == 0) {
            if (temp->stock > 0) {
                temp->stock--;  // הפחתת מלאי הספר
                addPurchase(customer, bookTitle); // הוספת הרכישה ללקוח
                printf("Purchase successful! '%s' has been added to %s's purchases.\n", bookTitle, customer->name);
                return;
            }
            else {
