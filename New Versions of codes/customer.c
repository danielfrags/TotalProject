    ...
    while (fgets(name, sizeof(name), file)) {
        ...
        for (int i = 0; i < purchaseCount; i++) {
            ...
            if (fgets(bookTitle, sizeof(bookTitle), file)) {
                ...
                addPurchase(newCustomer, bookTitle);
            }
        }
    }
...
void saveCustomerData(const char* filename, Customer* head) {
    ...
    while (current != NULL) {
        fprintf(file, "%s\n", current->name);
        fprintf(file, "%d/%d/%d\n", current->joinDate.tm_mday, current->joinDate.tm_mon + 1, current->joinDate.tm_year);
...
void viewCustomers(Customer* head) {
    ...
    while (current != NULL) {
        ...
        printf("Join Date: %d/%d/%d\n", current->joinDate.tm_mday,
            current->joinDate.tm_mon + 1, current->joinDate.tm_year);
...
void cusromer_menu() {
    ...
    // טעינת נתוני לקוחות מקובץ בעת הפעלת המערכת
